...
def log_and_print_status(balance, current_total_usd, total_gain_usd, trade_action=None, volume=None, price=None):
    ...
    status_message = (
        f"[{timestamp}] SOL@{balance.get('sol_price', 0.0):.2f} "
    ...
    if trade_action and volume is not None and price is not None:
        trade_message = (
            f"[{timestamp}] Trade action: {trade_action}, Volume: {volume:.6f}, Price: {price}, "
            f"New USDT Balance: {balance['usdt']:.2f}, New SOL Balance: {balance['sol']:.6f}"
        )
        print(trade_message)
...
def calculate_total_usd(balance):
    return balance['usdt'] + balance['sol'] * balance.get('sol_price', 0.0)
