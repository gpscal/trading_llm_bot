...
def calculate_indicators(btc_historical, sol_historical):
    ...
    def convert_to_python(obj):
        if hasattr(obj, 'item'):  # numpy scalar
            return obj.item()
        elif hasattr(obj, '__iter__') and not isinstance(obj, (str, bytes)):  # numpy array or similar
            return [convert_to_python(x) for x in obj]
        else:
            return obj

    btc_indicators = {
        'moving_avg': convert_to_python(calculate_moving_average(btc_historical)),
        'bollinger_bands': convert_to_python(calculate_bollinger_bands(btc_historical)),
        'macd': convert_to_python(calculate_macd(btc_historical)),
        'rsi': convert_to_python(calculate_rsi(btc_historical)),
        'atr': convert_to_python(calculate_atr(btc_historical)),
        'correlation': convert_to_python(calculate_correlation(btc_historical, sol_historical)),
        'stochastic_oscillator': convert_to_python(calculate_stochastic_oscillator(btc_historical)),
        'momentum': convert_to_python(calculate_momentum(btc_historical)),
        'adx': convert_to_python(calculate_adx(btc_historical)),
        'obv': convert_to_python(calculate_obv(btc_historical))
    }
    sol_indicators = {
        'moving_avg': convert_to_python(calculate_moving_average(sol_historical)),
        'bollinger_bands': convert_to_python(calculate_bollinger_bands(sol_historical)),
        'macd': convert_to_python(calculate_macd(sol_historical)),
        'rsi': convert_to_python(calculate_rsi(sol_historical)),
        'atr': convert_to_python(calculate_atr(sol_historical)),
        'correlation': convert_to_python(calculate_correlation(sol_historical, btc_historical)),
        'stochastic_oscillator': convert_to_python(calculate_stochastic_oscillator(sol_historical)),
        'momentum': convert_to_python(calculate_momentum(sol_historical)),
        'adx': convert_to_python(calculate_adx(sol_historical)),
        'obv': convert_to_python(calculate_obv(sol_historical))
    }
